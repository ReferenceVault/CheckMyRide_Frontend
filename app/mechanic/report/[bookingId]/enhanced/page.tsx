...
import {
  BodyConditionItem,
  RATING_OPTIONS,
  ENHANCED_BODY_CONDITION_ITEMS,
  LIGHTS_ITEMS,
  WHEELS_TIRES_ITEMS,
  ENHANCED_EXTERIOR_COMPONENTS_ITEMS,
  ENHANCED_UNDERCARRIAGE_ITEMS,
  ENGINE_ITEMS,
  BATTERY_ITEMS,
  ENHANCED_FLUID_INSPECTION_ITEMS,
  BELTS_HOSES_ITEMS,
  ENHANCED_DASHBOARD_CONTROLS_ITEMS,
...
export default function EnhancedInspectionPage() {
  ...
  const [selectedInspectionType, setSelectedInspectionType] = useState<string>('enhanced');
  ...
  const fetchBookingAndReport = async () => {
    try {
      ...
      setSelectedInspectionType(bookingData.booking.inspectionDetails?.type || 'enhanced');
      ...
      setFormData(prev => ({
        ...
        bodyCondition: ENHANCED_BODY_CONDITION_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        lights: LIGHTS_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        wheelsTires: WHEELS_TIRES_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        exteriorComponents: ENHANCED_EXTERIOR_COMPONENTS_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        undercarriage: ENHANCED_UNDERCARRIAGE_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        engine: ENGINE_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        battery: BATTERY_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        fluidInspection: ENHANCED_FLUID_INSPECTION_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        beltsHoses: BELTS_HOSES_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
        dashboardControls: ENHANCED_DASHBOARD_CONTROLS_ITEMS.map(item => ({ item, rating: '' as const, notes: '' })),
  ...
  const calculateFormProgress = (): number => {
    ...
    const sections: Array<keyof typeof formData> = [
      ...
      'battery', 'fluidInspection', 'beltsHoses', 'dashboardControls', 'windowsMirrors',
      'safetyEquipment', 'functionalTests', 'interiorCondition', 'seatsUpholstery',
      'drivingPerformance', 'diagnosticTesting'
    ];
